...
def refine_feature_high_res(original_mesh, features, feature_type="Pillar", margin=1.0, high_res=0.1):
    ...
    for index, feat in enumerate(features):
        ...
        # 【修改 1】：不仅提取形状，还要提取并继承传过来的具体工业参数 (Shape_Params)
...
# 【修改 1】：不仅提取形状，还要提取并继承传过来的具体工业参数 (Shape_Params)
        ...
        # 【新增】：把 shape_params 格式化成好看的字符串显示在屏幕上
        ...
        for i, slice_2d in enumerate(slices):
            ...
            for poly in slice_2d.polygons_closed:
                # 将多边形面积换算成真实半径，允许最大 3.0mm 的粗测误差
        ...
        if valid_depths:
            ...
            # 【修改 2】：把 shape_params 回写进字典结构，确保它一路传到最终的 v34 JSON 里
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*50}")
    print("[*] 启动高精度空间特征修正引擎 (全对称防呆 + 工业参数继承版)")
    print(f"{'='*50}")
